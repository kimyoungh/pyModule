...
def arrAR(data, c=3, p=24):
    ...
    aar = df(aar, index=data.index[p - 1:], columns=["AR"])
    ...
    for i in np.arange(len(wv)):
        if i == 0:
            for j in np.arange(wv.shape[1]):
                if wv[i, j] > 0:
                    rtemp = retv[i, j]
                    rtemp = np.exp(rtemp)-1
                    rtemp = rtemp - bfee
                    retv[i, j] = np.log(rtemp + 1)
                    #wv[i, j] *= (1 - bfee)
                else:
                    pass
            ...
            for j in np.arange(wv.shape[1]):
                if wv[i, j] > wv[i - 1, j]: # Buy more
                    rtemp = retv[i, j]
                    rtemp = np.exp(rtemp) - 1
                    rtemp = rtemp - bfee
                    retv[i, j] = np.log(rtemp + 1)
                    #wv[i, j] = -(wv[i, j] - wv[i - 1, j]) * bfee + wv[i, j]
                elif wv[i, j] < wv[i -1, j]: # Sell more
                    rtemp = retv[i - 1, j]
                    rtemp = np.exp(rtemp) - 1
                    rtemp = rtemp - sfee
                    retv[i - 1, j] = np.log(rtemp + 1)
                    #wv[i-1, j] = -(wv[i - 1, j] - wv[i, j]) * sfee + wv[i - 1, j]
                else:
                    pass
...
cpdef crebalancing(returns, wt, double bfee=0.0, double sfee=0.0):
    ...
    for i in np.arange(len(wv)):
        if i == 0:
            ...
            for j in np.arange(wv.shape[1]):
                if wv[i, j] > wv[i - 1, j]:
                    retv[i, j] = ((wv[i, j] - wv[i - 1, j]) * (1 + retv[i, j]) * (1 - bfee) - 1.) + (wv[i - 1, j] * (1 + retv[i, j])-1.)
                elif wv[i, j] < wv[i -1, j]:
                    retv[i, j] = ((wv[i - 1, j] - wv[i, j]) * (1 + retv[i, j]) * (1 - sfee) - 1.) + (wv[i, j] * (1 + retv[i, j]) - 1.) 
                else:
                    retv[i, j] = (1 + retv[i, j]) - 1.
...
cpdef chlogweights(returns, wt, double bfee=0.0, double sfee=0.0):
    ...
    for i in np.arange(len(wv)):
        if i == 0:
            ...
            for j in np.arange(wv.shape[1]):
                if wv[i, j] > wv[i - 1, j]:
                    retv[i, j] = ((wv[i, j] - wv[i - 1, j]) * np.exp(retv[i, j]) * (1 - bfee) - 1.) + (wv[i - 1, j] * np.exp(retv[i, j]) - 1.) 
                    retv[i, j] = np.log(retv[i, j] + 1.)
                elif wv[i, j] < wv[i -1, j]:
                    retv[i, j] = ((wv[i - 1, j] - wv[i, j]) * np.exp(retv[i, j]) * (1 - sfee) - 1.) + (wv[i, j] * np.exp(retv[i, j]) - 1.)
                    retv[i, j] = np.log(retv[i, j] + 1.)
                else:
                    retv[i, j] = np.exp(retv[i, j]) - 1.
                    retv[i, j] = np.log(retv[i, j] + 1.)
...
cpdef double portfolioTurbulence(np.ndarray returns, np.ndarray weights):
    ...
    """
    weights = weights.reshape(1, returns.shape[1])
...
def zpTurb(returns, weights, p = 60, zp = 60):
    """
        Turbulence index의 z value 시계열 만들기
        returns: 수익률 시계열 데이터프레임
        weights: 포트폴리오 구성종목들의 현재 투자비중 Spot
        p: turbulence index rolling window
        zp: rolling window for z value of turbulence index
    """

    rett = returns.values
    weights = weights.reshape(1, returns.shape[1])
    
    zpt = zportTurbulence(rett, weights, p=p, zp=zp)
    zpt = df(zpt, index=returns.index[p - 1:], columns=['PT'])
    zpt = (zpt - zpt.rolling(zp).mean())/zpt.rolling(zp).std(ddof=1)
    zpt = zpt.dropna()

    return zpt

cpdef np.ndarray zportTurbulence(np.ndarray rett, np.ndarray weights, int p = 60, int zp = 60):
    """
        Turbulence index의 z value 시계열 만들기
        returns: 수익률 시계열 데이터프레임
        weights: 포트폴리오 구성종목들의 현재 투자비중 Spot
    ...
    cdef np.ndarray zpt
    cdef int i
    zpt = np.zeros((len(rett) - p + 1, 1))
    for i in np.arange(len(rett) - p + 1):
        rets = rett[i:i+p]
        zpt[i] = portfolioTurbulence(rets, weights)
    return zpt

def normalscore(factor, ascend=True):
    """
    ...
    for i in np.arange(len(factor)):
        ftemp = factor.iloc[i]
        sfactor[i] = (len(ftemp) - ftemp.rank(ascending=ascend))/(len(ftemp)-1) * 100
